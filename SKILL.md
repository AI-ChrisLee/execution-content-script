...
description: Use this when the founder says "write the money script", "write my script", or "/the-money-driven-script", right after the winner file and the locked package exist. It writes the long-form YouTube script in 2 passes, a prep the founder reads first and then the script, on the 8-beat cage (hook, credibility, outline, problem, solution, workflow, FAQ, close), on one layer, the face or the screen with the webcam bubble on it. Written to make money, not views: it delivers the promise the title sold, puts your paid work on the screen, and makes one ask.
---
...
- Real, and safe to show. A demo screen carrying another party's data is either a demo account
  you own, a sanitised copy with names and account numbers changed, or shown with written
  permission. Say on screen which one it is. Sanitising is not faking; inventing a number is.
...
- `03_SCRIPT.md`, same folder. The 6 hook alt takes are its final section, never a separate file.
...
can prove on a screen, and every beat ends with what the viewer sees (the `See:` line; on the
screen layer the `[SCREEN:]` line heading the beat).
...
Ask the layer first, before anything else, unless the roots file carries a standing answer: face
(their face, camera only) or screen (their screen with the webcam bubble on it; a deck is a
screen, the live screen is a screen). Write `Layer: face` or `Layer: screen` at the top of
`00_PREP.md` and keep it for the whole video.

Then ask 2 things, in plain words:
...
- For every screen carrying another party's data, which of the 3 safe options it is. Settle it
...
- The first line under the title is the layer line, verbatim from the prep: `Layer: face` or
  `Layer: screen`. One per script.
- `[SCREEN: exact app or file, what is visible]` on the screen layer, heading every workflow beat
  and every screen flip, named exactly enough to stage before record. Those lines are the
  record-day screen list. On the face layer there are none.
...
The demo is the founder's paid work, inside the workflow beats. 2 ways: the recording they made
while doing the job (`demo.mp4`, named in `00_PREP.md`), played and talked over, or the job redone
live on camera. Every demo opens with the result, then shows how it was reached. When the demo is
a skill, the pacing is the triplet, narration between each pair:
...
The 6 hook alt takes close the file: 6 alternate first lines, 1 or 2 sentences each, anchoring the
number, the pain, the objection, the curiosity gap, the giveaway and the transformation. They
stand by for a hook swap if retention says the first missed.
...
The script is locked and nothing here edits it. Want slides? Install the-presentation and say
"My script is locked. Build the slides." Then record, then the cut.
...
- Mixing layers. A cut from the face to a screen and back. One layer, start to finish; on the
  screen layer the webcam bubble is the face.
