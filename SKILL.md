...
description: Use this when the founder says "Write the money script.", "write my script" or "/execution-content-script", when they type "The prep is approved.", or when they say "continue the money script". It reads the locked package, the winner file it names and squad/business.md, asks 2 things, writes 00_PREP.md in the episode folder, stops once, then writes 03_SCRIPT.md, the long-form YouTube script the founder reads on camera. It never records, posts or publishes.
---
...
- Never fake a screen, a number, a comment, a quote or a moment.
...
- Somebody else's data on screen: names and account numbers blurred.
...
1. Tell me 1 real moment when something went wrong, broke, or surprised you.
2. What on your screen shows the thing you sell working? A file, a recording, a dashboard.

No real moment, or no screen for ask 2: that spot reads NOT YET A BEAT in the prep. Never make one up.
...
7. **The real moment** and the screen that shows the work, from step 2.
8. **The voice.** The `voice sample` row is the writing sample. No sample: 1 line saying so, and the reference files are structure only.
...
Only after "The prep is approved." Write `<episode>/03_SCRIPT.md` off the approved prep. The format, worked: `references/standard-ep1-script.md`.
